...
func ErrorInvalidCompoundType(provided interface{}) error {
	return Error{
		...
		message: fmt.Sprintf("invalid type (got %s, expected %s, or a combination of these types (separated by |)", DataTypeUserStr(provided), strings.Join(s.UserStrs(append(ValueTypeStrings(), ColumnTypeStrings()...)), ", ")),
	}
}
...
func ErrorInvalidInputType(provided interface{}) error {
	return Error{
		...
		message: fmt.Sprintf("invalid type (got %s, expected %s, a combination of these types (separated by |), or a list or map containing these types", DataTypeUserStr(provided), strings.Join(s.UserStrs(append(ValueTypeStrings(), ColumnTypeStrings()...)), ", ")),
	}
}
...
func ErrorMustBeDefined(allowedType interface{}) error {
	return Error{
		...
		message: fmt.Sprintf("must be defined and fit the schema %s", DataTypeStr(allowedType)),
	}
}
